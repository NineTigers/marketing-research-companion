...
<header class="top"><div class="top-inner"><div class="brand">마케팅팀 대표 보고</div><div class="meta"><span class="badge ${job.mode === "codex" ? "live" : ""}">${modeLabel}</span><span class="badge">${statusLabel}</span><span class="badge">${escapeHtml(generatedAt)}</span><button class="print" type="button" onclick="window.print()">PDF 저장</button></div></div></header>
