...
function salesMethod(value) {
  return ({
    official_revenue: "공개 매출 자료",
    official_sales: "공식 판매 자료",
    order_count: "구매·주문 수",
    review_backcast: "리뷰 수 역산",
    traffic_backcast: "조회 수 역산",
    engagement_backcast: "댓글·관심 신호 역산",
    insufficient: "계산 근거 부족"
  })[value] || value || "추정 방식 확인 필요";
}
...
function clusterRows(items, sourceMap) {
  return (items || []).map((item) => `<tr>
    <td><strong>${escapeHtml(item.label)}</strong><small>${escapeHtml(item.mentionCount)}회 / ${escapeHtml(item.sampleSize)}건 · ${escapeHtml(item.sharePercent)}% · 신뢰도 ${escapeHtml(confidence(item.confidence))}</small><small>${escapeHtml((item.channels || []).join(" · "))} · ${escapeHtml(item.period)}</small></td>
...
function taskOutcomeSection(outcome, sourceMap, job, competitors) {
  if (!outcome) return "";
  const sourceCell = (item) => sourceLinks(item.sourceRefs, sourceMap);
  const visualCell = (item) => {
    const index = competitors.findIndex((competitor) => competitor.productUrl === item.productUrl);
    return index >= 0 ? competitorVisual(job.productVisuals?.[index], competitors[index]) : '<span class="source-missing">제품 연결 확인 필요</span>';
  };
  let body = "";
  if (outcome.recommendationItems?.length) body = `<div class="table-wrap"><table><thead><tr><th>제품 이미지</th><th>제품</th><th>가격</th><th>추천 판단</th><th>이유·trade-off</th><th>근거</th></tr></thead><tbody>${outcome.recommendationItems.map((item) => `<tr><td>${visualCell(item)}</td><td><strong>${escapeHtml(item.brand)} ${escapeHtml(item.product)}</strong><small>${isWebUrl(item.productUrl) ? `<a href="${escapeHtml(item.productUrl)}" target="_blank" rel="noreferrer">구매 페이지</a>` : "링크 확인 필요"}</small></td><td>${escapeHtml(item.price)}</td><td>${escapeHtml(item.decision)}</td><td>${escapeHtml(item.rationale)}<small>${escapeHtml(item.tradeoff)}</small></td><td class="sources">${sourceCell(item)}</td></tr>`).join("")}</tbody></table></div>`;
  else if (outcome.mdCandidates?.length) body = `<div class="table-wrap"><table><thead><tr><th>제품 이미지</th><th>후보</th><th>판정·수요</th><th>MOQ·원가</th><th>예상 마진</th><th>위험·다음 행동</th><th>근거</th></tr></thead><tbody>${outcome.mdCandidates.map((item) => `<tr><td>${visualCell(item)}</td><td><strong>${escapeHtml(item.brand)} ${escapeHtml(item.product)}</strong></td><td><strong>${escapeHtml(item.decision)}</strong><small>${escapeHtml(item.demand)}</small></td><td>${escapeHtml(item.moq)}<small>${escapeHtml(item.unitCost)}</small></td><td>${escapeHtml(item.expectedMargin)}</td><td>${escapeHtml(item.risk)}<small>${escapeHtml(item.nextAction)}</small></td><td class="sources">${sourceCell(item)}</td></tr>`).join("")}</tbody></table></div>`;
  else if (outcome.distributionCandidates?.length) body = `<div class="table-wrap"><table><thead><tr><th>제품 이미지</th><th>우선순위</th><th>후보</th><th>90일 GMV</th><th>예상 마진</th><th>협상 조건</th><th>위험·중단 조건</th><th>근거</th></tr></thead><tbody>${outcome.distributionCandidates.map((item) => `<tr><td>${visualCell(item)}</td><td>${escapeHtml(item.rank)}</td><td><strong>${escapeHtml(item.brand)} ${escapeHtml(item.product)}</strong></td><td>${escapeHtml(item.ninetyDayGmv)}</td><td>${escapeHtml(item.expectedMargin)}</td><td>${escapeHtml(item.negotiationTerms)}</td><td>${escapeHtml(item.claimRisk)}<small>${escapeHtml(item.stopCondition)}</small></td><td class="sources">${sourceCell(item)}</td></tr>`).join("")}</tbody></table></div>`;
  else if (outcome.evidenceChecks?.length) body = `<div class="table-wrap"><table><thead><tr><th>제품 이미지</th><th>제품</th><th>판정</th><th>일치</th><th>불일치·미확인</th><th>근거</th></tr></thead><tbody>${outcome.evidenceChecks.map((item) => `<tr><td>${visualCell(item)}</td><td><strong>${escapeHtml(item.brand)} ${escapeHtml(item.product)}</strong></td><td>${escapeHtml(item.status)}</td><td>${escapeHtml(item.matched.join(" · "))}</td><td>${escapeHtml(item.mismatches.join(" · "))}</td><td class="sources">${sourceCell(item)}</td></tr>`).join("")}</tbody></table></div>`;
  else body = `<div class="band"><h3>우선 기회</h3>${list(outcome.opportunities)}</div>`;
  return `<section class="section"><div class="section-head"><h2>이번 업무의 결론</h2><p>${escapeHtml(outcome.summary)}</p></div>${body}</section>`;
}

function competitorVisual(visual, item) {
  const official = visual?.official?.url || (isWebUrl(item.officialImageUrl) ? item.officialImageUrl : "");
  const generated = visual?.generated?.url || "";
  if (!official && !generated) return '<span class="source-missing">제품 이미지 확인 필요</span>';
  return `<div class="visual-pair">${official ? `<figure><img src="${escapeHtml(official)}" alt="${escapeHtml(item.brand)} ${escapeHtml(item.product)} 공식 제품 이미지"><figcaption>공식 판매 이미지</figcaption></figure>` : ""}${generated ? `<figure><img src="${escapeHtml(generated)}" alt="${escapeHtml(item.brand)} ${escapeHtml(item.product)} 동일 제품 사용 장면"><figcaption>동일 제품 사용 장면</figcaption></figure>` : ""}</div>`;
}
...
export function renderReport({job, report, sources, quality}) {
  ...
  const competitorRows = report.competitors.map((item, index) => {
    ...
    const units = estimate.calculated?.monthlyUnits?.base;
    return `<tr><td>${competitorVisual(job.productVisuals?.[index], item)}</td><td><strong>${escapeHtml(item.brand)}</strong><span>${escapeHtml(item.product)}</span><small>${escapeHtml(item.checkedAt)} · ${productLink}</small></td><td>${escapeHtml(item.price)}<small>${escapeHtml(item.reviewSignal)}</small></td><td>${escapeHtml(item.successFactors.join(" · "))}</td><td><strong>${escapeHtml(estimate.base)}</strong><small>${escapeHtml(estimate.low)} / ${escapeHtml(estimate.high)}</small><small>기준 월 판매량 ${escapeHtml(units ?? "확인 필요")}개</small><small>${escapeHtml(salesMethod(estimate.method))} · ${escapeHtml(estimate.period || "기간 확인 필요")}</small><small>${escapeHtml(estimate.formula)}</small><small>가격: ${escapeHtml(estimate.priceBasis || "확인 필요")}</small><small>입력: ${escapeHtml((estimate.inputs || []).join(" · "))}</small><small>신호: ${escapeHtml((estimate.demandSignals || []).join(" · "))}</small><div class="sources">${sourceLinks(estimate.sourceRefs, sourceMap)}</div></td><td>${escapeHtml(item.weakness)}</td><td class="sources">${sourceLinks(item.sourceRefs, sourceMap)}</td></tr>`;
  }).join("");
...
${taskOutcomeSection(report.taskOutcome, sourceMap, job, report.competitors)}
${report.charts?.length ? `<section class="section"><div class="section-head"><h2>핵심 근거 차트</h2><p>대표가 선택한 근거별 표현 방식을 실제 조사 수치에 적용했습니다.</p></div><div class="chart-grid">${renderCharts(report.charts, (refs) => sourceLinks(refs, sourceMap))}</div></section>` : ""}
...
<section class="section"><div class="section-head"><h2>경쟁 제품 비교</h2><p>공식 이미지·동일 제품 사용 장면·판매 URL·가격·리뷰·검산된 매출 추정을 같은 제품 행에서 비교했습니다.</p></div><div class="table-wrap"><table><thead><tr><th>제품 이미지</th><th>브랜드·제품</th><th>가격·리뷰</th><th>성공 요인</th><th>월매출 추정</th><th>약점</th><th>출처</th></tr></thead><tbody>${competitorRows}</tbody></table></div></section>
<section class="section"><div class="section-head"><h2>고객 반응</h2><p>${escapeHtml(report.voc.sampleNote)} · 표본 ${escapeHtml(report.voc.sampleSize)}건 · ${escapeHtml(report.voc.collectionPeriod)} · ${escapeHtml((report.voc.channels || []).join(" · "))}</p></div><div class="two"><div><h3>만족 포인트</h3><div class="table-wrap"><table><thead><tr><th>주제·빈도</th><th>고객 언어</th><th>숨은 필요</th><th>제품 반영</th><th>출처</th></tr></thead><tbody>${clusterRows(report.voc.satisfaction, sourceMap)}</tbody></table></div></div><div><h3>불만 포인트</h3><div class="table-wrap"><table><thead><tr><th>주제·빈도</th><th>고객 언어</th><th>숨은 필요</th><th>제품 반영</th><th>출처</th></tr></thead><tbody>${clusterRows(report.voc.dissatisfaction, sourceMap)}</tbody></table></div></div></div></section>
<section class="section"><div class="section-head"><h2>성공 원인</h2><p>인기라는 결과 대신 제품·가격·채널·마케팅 요인을 분해했습니다.</p></div><div class="cause-list">${report.successCauses.map((item) => `<article class="cause"><h3>${escapeHtml(item.cause)} <small>${escapeHtml(item.type)}</small></h3><p><strong>근거</strong> ${escapeHtml(item.evidence)}</p><p><strong>우리의 적용</strong> ${escapeHtml(item.implication)}</p><div class="sources">${sourceLinks(item.sourceRefs, sourceMap)}</div></article>`).join("")}</div></section>
<section class="section"><div class="section-head"><h2>구매 고객</h2><p>연령·발달 단계와 구매 목적을 함께 정의했습니다.</p></div><div class="persona-list">${report.personas.map((item) => `<article class="persona"><h3>${escapeHtml(item.name)}</h3><p><strong>${escapeHtml(item.ageStage)}</strong> · ${escapeHtml(item.buyer)} · ${escapeHtml(item.purpose)}</p><p><strong>우선순위</strong> ${escapeHtml(item.priorities.join(" · "))}</p><p><strong>불안</strong> ${escapeHtml(item.anxieties.join(" · "))}</p><p><strong>필요한 증거</strong> ${escapeHtml(item.proofNeeds.join(" · "))}</p><div class="sources">${sourceLinks(item.sourceRefs, sourceMap)}</div></article>`).join("")}</div></section>
...
export {escapeHtml};
import {renderCharts} from "./chart-renderer.mjs";
