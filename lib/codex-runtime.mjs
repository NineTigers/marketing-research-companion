...
export class CodexRuntime extends EventEmitter {
  ...
  async #startProcess() {
    ...
    await this.request("initialize", {
      clientInfo: {name: "marketing_research_companion", title: "Marketing Research Companion", version: "3.3.0"},
  ...
  async startChatGptLogin() {
    return this.request("account/login/start", {type: "chatgpt", useHostedLoginSuccessPage: true, appBrand: "chatgpt"}, 30000);
  }

  async cancelLogin(loginId) {
    if (!loginId) throw Object.assign(new Error("취소할 로그인 요청이 없습니다."), {statusCode: 400});
    return this.request("account/login/cancel", {loginId}, 30000);
  }
    ...
    try {
      ...
      const started = await this.request("turn/start", {
        ...
        input: [
          ...
          ...referenceImagePaths.filter((item) => typeof item === "string" && path.isAbsolute(item)).slice(0, 5).map((imagePath) => ({type: "localImage", path: imagePath}))
        ],
        effort
      });
      turnId = started.turn.id;
