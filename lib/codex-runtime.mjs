...
export class CodexRuntime extends EventEmitter {
  ...
  async #startProcess() {
    ...
    await this.request("initialize", {
      clientInfo: {name: "marketing_research_companion", title: "Marketing Research Companion", version: "2.1.0"},
  ...
  async status({requiredModel = null} = {}) {
    try {
      const account = await this.getAccount();
      const connected = account?.type === "chatgpt";
      let modelAvailable = null;
      if (connected && requiredModel) {
        const models = await this.listModels();
        modelAvailable = models.some((item) => item.id === requiredModel || item.model === requiredModel);
      }
      ...
      return {
        ...
        requiredModel,
        modelAvailable,
        reason: modelAvailable === false ? `${requiredModel} 모델을 현재 Codex 계정에서 사용할 수 없습니다.` : (account?.type === "apiKey" ? "ChatGPT 계정 로그인이 필요합니다." : (account ? null : "ChatGPT 계정을 연결해 주세요."))
      };
    } catch (error) {
      return {installed: false, connected: false, ready: false, accountType: null, email: null, planType: null, requiredModel, modelAvailable: null, reason: error.message, details: error.details || ""};
    }
  }
