...
export class CodexRuntime extends EventEmitter {
  ...
  async #startProcess() {
    ...
    await this.request("initialize", {
      clientInfo: {name: "marketing_research_companion", title: "Marketing Research Companion", version: "2.0.0"},
  ...
  async status() {
    try {
      ...
      return {
        ...
        reason: account?.type === "apiKey" ? "ChatGPT 계정 로그인이 필요합니다." : (account ? null : "ChatGPT 계정을 연결해 주세요.")
      };
    } catch (error) {
      return {installed: false, connected: false, accountType: null, email: null, planType: null, reason: error.message, details: error.details || ""};
    }
  }
  ...
  async runStructured({prompt, outputSchema, cwd = this.cwd, model = null, effort = "medium", signal, onEvent}) {
