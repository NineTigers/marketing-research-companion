...
export class FileStore {
  ...
  async writeGeneratedImage(id, generated, baseName = "proposal-image") {
    const reportId = safeId(id);
    const targetDir = this.assetDir(reportId);
    await mkdir(targetDir, {recursive: true});
    const safeBaseName = String(baseName || "proposal-image").replace(/[^a-zA-Z0-9_-]/g, "-").slice(0, 80) || "proposal-image";
    ...
    if (sourcePath && path.isAbsolute(sourcePath)) {
      ...
      const name = safeBaseName + extension;
  ...
  async writeJobAssetBuffer(id, name, buffer) {
    const targetDir = this.assetDir(id);
    await mkdir(targetDir, {recursive: true});
