...
function initialAgents() {
  return AGENT_DEFINITIONS.map((agent) => ({...agent, status: "waiting", startedAt: null, completedAt: null, note: ""}));
}
...
export function createJobRecord(request, config) {
  ...
  return {
    ...
    status: "queued",
    mode: config.mode,
    model: config.mode === "codex" ? (config.model || "account-default") : "deterministic-demo",
    request,
...
function qualityReview(report, sources, mode) {
  const warnings = [];
  const checks = [];
...
export async function runResearchJob({jobId, store, provider, signal}) {
  ...
  try {
    ...
    const quality = qualityReview(report, sources, starting.mode);
    quality.warnings.push(...safetyWarnings);
    ...
    const artifacts = await store.writeReport(jobId, html, {request, report, sources, quality, mode: starting.mode, model: starting.model});
