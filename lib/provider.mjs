...
export class CodexProvider {
  ...
  async generateProductImage({request, report, signal}) {
    const selectedModel = await this.resolveModel();
    const proposal = report.productProposal;
    const prompt = [
      "이미지 생성 도구를 정확히 한 번 사용해 자사 제안 상품의 실사용 검토 이미지를 만드세요.",
      "사진처럼 사실적인 단일 장면으로 만들고 제품 전체 형태, 크기, 소재와 사용 맥락이 한눈에 보여야 합니다.",
      `제품군: ${request.product}`,
      `대상 고객과 사용 맥락: ${proposal.targetUser} / ${request.context || request.stage}`,
      `상품 콘셉트: ${proposal.concept}`,
      `반드시 구현할 스펙: ${(proposal.requiredSpecs || []).join(", ")}`,
      `선택 스펙: ${(proposal.optionalSpecs || []).join(", ") || "없음"}`,
      "문자, 로고, 인증 마크, 의료 효능 표현을 이미지에 넣지 마세요.",
