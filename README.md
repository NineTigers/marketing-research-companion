...
VOC 분석, 업무별 상품·가격 제안, 공식 제품 이미지와 대표 보고서 생성을 실행하는 개인용 데스크톱 앱입니다.
같은 제품을 소스형 로컬 웹 서비스로도 실행할 수 있습니다.
...
- 배포 형태: 사용자별 설치형 데스크톱 컴패니언 또는 소스형 로컬 웹 서비스
...
- 저장 위치: 설치 소스와 분리된 운영체제 사용자 데이터 디렉터리
- 네트워크: 기본값은 `127.0.0.1`; 데스크톱은 앱 전용, 소스형 웹만 SSH 포트 포워딩 지원

여러 사람이 하나의 중앙 서버에 로그인하는 SaaS가 아닙니다. 여러 사용자는 각자 이
저장소를 설치하고 자신의 ChatGPT 계정을 연결합니다.

이 저장소가 제품 소스, 공개 문서, 테스트, CI와 릴리스를 직접 소유합니다. Workbench의
`asset` 또는 `dist`에서 실행본을 내보내는 중간 단계는 사용하지 않습니다.
...
첫 배포 대상은 Apple Silicon macOS입니다. 개발 설치물은
`release/Marketing Research Companion-3.3.0-arm64.dmg`입니다.

1. DMG를 열고 **Marketing Research Companion**을 **Applications**로 옮깁니다.
2. 앱 아이콘으로 실행합니다. 별도 터미널과 Node.js 설치는 필요하지 않습니다.
3. 필요하면 상단의 **ChatGPT로 로그인**을 누릅니다.
4. 시스템 브라우저에서 본인 계정으로 승인한 뒤 앱으로 돌아옵니다.

현재 DMG는 기능 검증용 unsigned 개발 설치물입니다. macOS가 첫 실행을 막으면 Finder에서
앱을 Control-클릭한 뒤 **열기**를 선택해야 합니다. 일반 고객 배포는 Developer ID 서명과
Apple 공증을 완료한 별도 릴리스로 제공해야 합니다.

앱은 설치된 Codex를 먼저 찾습니다. ChatGPT macOS 앱에 포함된 Codex 또는 호환 Codex
CLI가 없으면 진단 상태를 보여주며, 실행 파일을 몰래 설치하거나 계정 정보를 대신 입력하지 않습니다.

## Requirements

### 데스크톱 설치물

- Apple Silicon macOS 12 이상
- ChatGPT 데스크톱 앱에 포함된 Codex 또는 호환 Codex CLI
...
자격 증명을 재사용합니다. 로그아웃 상태라면 화면의 **ChatGPT로 로그인**을 눌러 브라우저
로그인을 완료합니다.
...
소스형 설치 후에는 웹의 **운영 참고 → 서비스 관리**에서 **업데이트 확인**과 **업데이트**를
...
데스크톱 설치물은 앱 번들 안의 Git 파일을 직접 수정하지 않습니다. 현재 개발 설치물에는
고객 자동 업데이트가 없으며, 이후 서명된 패키지 배포 채널로 제공할 예정입니다.

Codex에게 Git 링크와 함께 설치를 맡길 때 사용할 간단한 지시는 다음과 같습니다.

```text
현재 운영체제에서 Git, Node.js 22.12 이상, Codex 요구사항을 먼저 확인해줘.
...
Browser -> local Node server -> Codex App Server -> user's ChatGPT account
                         |-> OS user-data jobs, reports and assets
```
...
npm start       # 로컬 서비스 시작
npm run desktop:dev   # 소스에서 데스크톱 앱 실행
npm run desktop:smoke # 데모 조사·보고서·저장 자동 검증
npm run desktop:build # macOS arm64 unsigned DMG·ZIP 생성
...
마세요. 포트 포워딩이 필요하면 데스크톱 설치물이 아니라 소스형 웹 서비스를 사용하고
`ssh -L 8787:127.0.0.1:8787 <host>`처럼 연결한 뒤 [SECURITY.md](SECURITY.md)를 확인하세요.
데스크톱 로컬 서버는 앱이 보유한 임의 세션 값 없이는 접근을 거부합니다. VOC와 내부 상품 계획은 운영체제의
사용자 데이터 디렉터리에 저장되므로 운영체제 계정과 디스크 권한으로 보호해야 합니다.

기존 `asset/dist` 설치본의 이력을 옮길 때는
[정식 프로젝트 전환 안내](docs/formal-project-migration-ko.md)를 확인하세요.
