...
VOC 분석, 상품·가격 제안, 선택형 상품 사용 장면과 대표 보고서 생성을 실행하는 로컬 웹 서비스입니다.
...
조사 옵션의 **자사 제안 상품 사용 장면 생성**은 기본 해제입니다. 선택하면 Codex App
Server의 이미지 생성 capability를 확인한 뒤 자사 제안 스펙 기반 이미지 한 장을 생성해
보고서에 포함합니다. 별도 OpenAI API 키를 요구하지 않으며, capability가 없는 계정에서는
선택할 수 없습니다.
...
댓글·좋아요·저장 신호 순으로 계산 근거를 선택합니다. 공식 가격이 없으면 현재 판매
페이지 표시 가격, 동일 SKU 가격, 동일 채널 가격 범위를 차례로 사용하며 추정값과 공식
수치를 구분합니다.
...
- `GET /api/jobs/:id/assets/:name`: 선택 생성한 작업별 보고서 이미지
